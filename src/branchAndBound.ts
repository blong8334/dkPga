...
export function getBestLineup(
  ...
  const currLineup = { totalSal: 0, totalScore: 0, lineup: [] };
  let line_count = 1;
  let start = new Date();
  ...
  function branchAndBound(
    ...
    addThePlayerLineup.totalScore += +nextPlayer.score;
    ...
    if (nextCap < 0) {
      ...
      if (addThePlayerLineup.totalScore > line_queue[0].totalScore) {
      ...
      if (!remZScore) {
        ...
        let nextBest = addThePlayerLineup.totalScore + remZScore;
        if (nextBest < line_queue[0].totalScore) {
...
function findFirstBest(players: t_players, salaryCap: number, totalPlayerCount: number): t_bestLineup {
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    if (bestLineup.lineup.length === totalPlayerCount && bestLineup.totalSal > salaryCap) {
      ...
      bestLineup.totalScore -= leavingPlayer.score;
